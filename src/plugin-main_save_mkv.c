...
#include <windows.h>
#include <psapi.h>
#include <process.h> // 需要包含此头文件以使用 _beginthreadex
...
unsigned __stdcall MoveFileThread(void *param)
{
	...
	// 最大等待时间：5分钟 (300 * 1000 毫秒)，封装大文件可能需要一点时间
	int retries = 300;
	while (retries > 0) {
	...
	// 3. 移动原始文件（如果它还存在的话。有时OBS设置了封装后删除原文件）
	if (GetFileAttributesW(data->replay_path) != INVALID_FILE_ATTRIBUTES) {
		move_file_to_new_location(data->replay_path, data->window_name);
	}

	// 如果生成了 mp4，且原文件不是 mp4（防止重复移动），则移动 mp4 文件
	if (wcsicmp(ext, L".mp4") != 0 &&
	    GetFileAttributesW(mp4_path) != INVALID_FILE_ATTRIBUTES) {
		move_file_to_new_location(mp4_path, data->window_name);
	}
...
static void on_frontend_event(enum obs_frontend_event event, void *data)
{
	...
	// 创建后台线程执行移动操作，不阻塞 OBS 主线程
	HANDLE hThread = (HANDLE)_beginthreadex(NULL, 0, MoveFileThread,
						task_data, 0, NULL);
	if (hThread) {
		CloseHandle(hThread); // 线程独立运行，我们不需要等待它结束
	} else {
